"use client"
import { Deals, properties } from "@/app/components/Data";
...
import Stays from "@/app/components/Stays";
import Swipper from "@/app/components/swipper";
import Slider from "@/app/components/swipper";
import { Context } from "@/app/Context";
import { FilterProvider, useAppContext } from "@/app/FilterReducer";
import { format } from "date-fns";
import Link from "next/link";
import { BiDiamond, BiDrink, BiHeart, BiLike, BiRestaurant, BiWalk, BiWifi } from "react-icons/bi";
import { BsSuitDiamond } from "react-icons/bs";
import { FaBed, FaSwimmer } from "react-icons/fa";
import { ImLocation } from "react-icons/im";
import { IoArrowForwardOutline, IoDiamond, IoPeopleSharp } from "react-icons/io5";
import { MdArrowBackIosNew, MdArrowForward, MdArrowForwardIos, MdFreeBreakfast, MdOutlinePets, MdSpa } from "react-icons/md";
interface Params {
    PropertyId: number;
}
    ...
    return (
            <div id="overview" className="container mx-auto grid grid-cols-4 gap-4 p-6">
                ...
                <div className="hidden lg:block col-span-2">
                    <img className="w-full h-80 rounded-lg" src={data[0].Img[0]} alt={data[0].Name} />
                </div>
                <div className="hidden lg:block col-span-1">
                    <img className="w-full h-40 rounded-lg p-1" src={data[0].Img[1]} alt={data[0].Name} />
                    <img className="w-full h-40 rounded-lg p-1" src={data[0].Img[2]} alt={data[0].Name} />

                </div>
                <div className="hidden lg:block col-span-1">
                    <img className="w-full h-40 rounded-lg p-1" src={data[0].Img[3]} alt={data[0].Name} />
                    <img className="w-full h-40 rounded-lg p-1" src={data[0].Img[4]} alt={data[0].Name} />
                </div>
                <div className="col-start-1 col-end-7 lg:col-span-2 border-b-[2px] sticky top-0 shadow bg-white z-50 p-2 rounded">
